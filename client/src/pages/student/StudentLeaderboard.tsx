...
const StudentLeaderboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSubjects = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:5000/api/subjects/student/${currentStudentId}/all`);
  ...
  useEffect(() => {
    ...
    const fetchLeaderboard = async () => {
      ...
      try {
        let url = `http://localhost:5000/api/analytics/leaderboard?subjectId=${subjectId}`;
