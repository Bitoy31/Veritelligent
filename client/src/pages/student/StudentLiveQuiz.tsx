...
const StudentLiveQuiz: React.FC = () => {
  ...
  const createAttempt = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/analytics/attempts', {
        method: 'POST',
  ...
  const submitAttempt = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/analytics/attempts/${currentAttemptId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      s.on('game_restarted', () => {
        ...
        if (currentAttemptId && !hasSubmittedAttempt.current) {
          try {
            const baseStart = (gameStartTimeRef.current || questionStartTime);
            const timeSpent = Math.round((Date.now() - baseStart) / 1000);
            fetch(`http://localhost:5000/api/attempts/${currentAttemptId}`, {
              method: 'PATCH',
              headers: { 'Content-Type': 'application/json' },
