...
const StudentDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      try {
        ...
        const response = await fetch(`https://api.veritelligent.fun/api/subjects/student/${user._id}/all`);
  ...
  const removeEnrollment = async (subjectId: string) => {
    try {
      const response = await fetch(`https://api.veritelligent.fun/api/subjects/${subjectId}/enrolled/${JSON.parse(localStorage.getItem('user')||'{}')._id}`, {
        method: 'DELETE'
      });
  ...
  const handleJoinById = async () => {
    ...
    try {
      ...
      const response = await fetch('https://api.veritelligent.fun/api/join/join-by-id', {
        method: 'POST',
        headers: {
