...
const StudentSessions: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSubjects = async () => {
      try {
        const res = await fetch(`https://api.veritelligent.fun/api/subjects/student/${currentStudentId}/all`);
        if (res.status === 401) {
          localStorage.clear();
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const attemptsRes = await fetch(`https://api.veritelligent.fun/api/analytics/attempts?studentId=${currentStudentId}`);
        ...
        const sessionsRes = await fetch(`https://api.veritelligent.fun/api/analytics/sessions?sessionIds=${sessionIds.join(',')}`);
