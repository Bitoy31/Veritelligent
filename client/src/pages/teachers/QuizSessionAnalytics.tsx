...
const QuizSessionAnalytics: React.FC = () => {
  ...
  const fetchSessionData = async () => {
    try {
      ...
      const response = await fetch(`https://api.veritelligent.fun/api/analytics/sessions?taskId=${taskId}&subjectId=${subjectId}`);
      if (!response.ok) {
  ...
  const fetchAttemptsData = async () => {
    try {
      ...
      const response = await fetch(`https://api.veritelligent.fun/api/analytics/attempts?sessionId=${sessionId}`);
      if (!response.ok) {
        throw new Error(`HTTP ${response.status}: ${response.statusText}`);
      }
  ...
  const fetchQuizData = async () => {
    try {
      ...
      const response = await fetch(`https://api.veritelligent.fun/api/quiz/${taskId}`);
      if (response.ok) {
