...
const GridQuestManagement: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch tasks
        const tasksRes = await fetch('https://api.veritelligent.fun/api/gridquest/tasks');
        if (tasksRes.ok) {
  ...
  const handleStatusChange = async (taskId: string, newStatus: 'draft' | 'published' | 'closed') => {
    try {
      const response = await fetch(`https://api.veritelligent.fun/api/gridquest/tasks/${taskId}/status`, {
        method: 'PATCH',
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`https://api.veritelligent.fun/api/gridquest/tasks/${selectedTask._id}`, {
        method: 'DELETE'
      });
...
const CreateGridQuestForm: React.FC<CreateGridQuestFormProps> = ({ subjects, onSave, onCancel }) => {
  ...
  const handleSave = async () => {
    ...
    const body = {
      ...
      const res = await fetch('https://api.veritelligent.fun/api/gridquest/tasks', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body)
      });
