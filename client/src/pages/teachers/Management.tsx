...
const Management: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch(`https://api.veritelligent.fun/api/subjects?teacherId=${teacher._id}`)
      .then(res => res.json())
    ...
    fetch(`https://api.veritelligent.fun/api/subjects?teacherId=${teacher._id}`)
      ...
      .then(async (subjects) => {
        ...
        for (const subj of subjects) {
            const res = await fetch(`https://api.veritelligent.fun/api/subjects/${subj._id}/enrolled`);
  ...
  const handleShowStudents = async (subject: any) => {
    ...
    setStudentsList([]);
    try {
      const res = await fetch(`https://api.veritelligent.fun/api/subjects/${subject._id}/enrolled`);
            ...
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              ...
              <form
                onSubmit={async (e) => {
                  ...
                  try {
                    const res = await fetch('https://api.veritelligent.fun/api/subjects', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify(payload),
                    });
                    if (!res.ok) throw new Error('Failed to add subject');
                    const updated = await fetch(`https://api.veritelligent.fun/api/subjects?teacherId=${teacher._id}`);
                      ...
                      <div className="mgmt-card-actions" onClick={(e) => e.stopPropagation()}>
                        ...
                        <button
                          ...
                          onClick={async (e) => {
                            ...
                            try {
                              const res = await fetch(`https://api.veritelligent.fun/api/subjects/${subj._id}`, {
                                method: 'DELETE',
                              });
                      ...
                      <div className="mgmt-card-actions">
                        {student.status === 'pending' && (
                          <button
                            ...
                            onClick={async () => {
                              const res = await fetch(`https://api.veritelligent.fun/api/subjects/${student.subjectId}/enrolled/${student.studentId}`, {
                                method: 'PATCH',
                                headers: { 'Content-Type': 'application/json' },
                ...
                <>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      const res = await fetch(`https://api.veritelligent.fun/api/subjects/${selectedSubject._id}`, {
                        method: 'PATCH',
                        headers: { 'Content-Type': 'application/json' },
