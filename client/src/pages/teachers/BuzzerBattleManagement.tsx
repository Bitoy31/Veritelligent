...
const BuzzerBattleManagement: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch tasks
      const tasksRes = await fetch(`http://localhost:5000/api/buzzerbattle/tasks?teacherId=${user._id}`);
      if (tasksRes.ok) {
  ...
  const handleUpdateTask = async (taskData: any) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/buzzerbattle/tasks/${selectedTask._id}`, {
        method: 'PATCH',
  ...
  const handleStatusChange = async (taskId: string, newStatus: 'draft' | 'published' | 'closed') => {
    try {
      const response = await fetch(`http://localhost:5000/api/buzzerbattle/tasks/${taskId}/status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
...
const CreateBuzzerBattleForm: React.FC<CreateBuzzerBattleFormProps> = ({ subjects, onSave, onCancel }) => {
  ...
  const handleCreate = async (taskData: any) => {
    ...
    try {
      const user = JSON.parse(localStorage.getItem('user') || '{}');
      const response = await fetch('http://localhost:5000/api/buzzerbattle/tasks', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...taskData,
