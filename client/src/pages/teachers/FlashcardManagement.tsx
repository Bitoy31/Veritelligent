...
const FlashcardManagement: React.FC = () => {
  ...
  const handleCreateTask = async (taskData: any) => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/flashcard/tasks', {
        method: 'POST',
  ...
  const handleUpdateTask = async (taskData: any) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/flashcard/tasks/${selectedTask._id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(taskData)
      });
  ...
  const handleStatusChange = async (taskId: string, newStatus: 'draft' | 'published' | 'closed') => {
    try {
      const response = await fetch(`http://localhost:5000/api/flashcard/tasks/${taskId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
