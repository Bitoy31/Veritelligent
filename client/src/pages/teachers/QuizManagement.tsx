...
const QuizManagement: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch quizzes
        const quizResponse = await fetch(`https://api.veritelligent.fun/api/quiz/teacher/${user._id}`);
        const quizData = await quizResponse.json();
        setQuizzes(quizData);

        // Fetch subjects
        const subjectResponse = await fetch(`https://api.veritelligent.fun/api/subjects?teacherId=${user._id}`);
  ...
  const handleStatusChange = async (quizId: string, newStatus: string) => {
    try {
      const response = await fetch(`https://api.veritelligent.fun/api/quiz/${quizId}/status`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
